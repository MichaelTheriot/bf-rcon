...
const handlers = {
  ...
  onDataCommand(data) {
    if (this.authenticated) {
      ...
      if (!s.queue.length) {
        return this.destroy(new RconError(`Unrequested data received from server: ${data}`, RconError.ERR_UNREQUESTED));
      }
  ...
  onClose() {
    ...
    this
      .removeListener('data', handlers.onDataDigest)
      .removeListener('data', handlers.onDataAuth);
  }
...
const RconConnection = class extends net.Socket {
  constructor() {
    ...
    this
      ...
      .on('data', handlers.onDataCommand);
  }
  ...
  async send(data) {
    ...
    try {
      ...
      if (!msg.length) {
        ...
        throw new RconError(`Unknown response format: "${msg}"`, RconError.ERR_UNKNOWN);
      }
    } catch (err) {
      this.destroy();
      throw err;
